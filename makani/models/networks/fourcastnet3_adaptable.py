...
# helpers
from makani.models.common import DropPath, LayerScale, MLP, EncoderDecoder, SpectralConv, GLUAdaptableSpectralConv
...
class NeuralOperatorBlock(nn.Module):
    def __init__(
        ...
        if conv_type == "local":
            ...
            self.global_conv = GLUAdaptableSpectralConv(spectral_conv)
        else:
    ...
    def forward(self, x, coarsening_level=0):
        """
        Updated NO block
        """

        if hasattr(self, "global_conv"):
            dx, _ = self.global_conv(x, coarsening_level=coarsening_level)
...
class AtmoSphericNeuralOperatorNet(nn.Module):
    ...
    def processor_blocks(self, x, x_aux, coarsening_level=0):
        ...
        for blk in self.blocks:
            ...
            if self.checkpointing_level >= 3:
                x = checkpoint(blk, x, coarsening_level, use_reentrant=False)
            else:
                x = blk(x, coarsening_level)
    ...
    def forward(self, x, coarsening_level=0):
        ...
        # run the processor
        x = self.processor_blocks(x, x_aux, coarsening_level=coarsening_level)
